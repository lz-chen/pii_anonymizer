...
TEXT_EN_INFO = [
    {"end": 25, "entity": "Jamie Clark", "entity_type": "PERSON", "start": 14},
...
class TestPredictor(unittest.TestCase):
    ...
    def test_anonymize_text_tag(self, text, lang, expected):
        anonymized_text = self.pii_anonymizer.anonymize_text(
            text, mode=TAGGED_TEXT_MODE, lang=lang
        )[TAGGED_TEXT_MODE]
    ...
    def test_anonymize_text_tag_other_lang(self):
        ...
        self.assertEqual(
            "Support for language es is not implemented yet!", context.exception.args[0]
        )

    def test_anonymize_es_text_tag_other_lang(self):
        text = "No te lo creo ni tantito."
        with self.assertRaises(ValueError) as context:
            anonymized_text = self.pii_anonymizer.anonymize_text(
                text, mode=TAGGED_TEXT_MODE, lang="unknown"
            )[TAGGED_TEXT_MODE]

        self.assertTrue(context.exception.args[0].endswith("not implemented yet!"))
    ...
    def test_anonymize_text_detail(self, text, lang, expected):
        ...
        anonymized_text = self.pii_anonymizer.anonymize_text(
            text, mode=DETAIL_INFO_MODE, lang=lang
        )[DETAIL_INFO_MODE]
    ...
    def test_anonymize_text_replaced(self):
        ...
        with self.assertRaises(ValueError) as context:
            anonymized_text = self.pii_anonymizer.anonymize_text(
                text, mode=REPLACED_TEXT_MODE
            )
        self.assertEqual(
            "Support for replaced_text mode is not implemented yet!",
